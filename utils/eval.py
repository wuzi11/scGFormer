...
def evaluate(model, dataset, split_idx, eval_func, criterion, args, result=None):
    """
    Evaluate the model on train, valid, and test splits.
    Additionally computes precision, recall, and F1-score.
    """
    device = torch.device("cuda:" + str(args.device)) if torch.cuda.is_available() else torch.device("cpu")
    if result is not None:
        out = result
    ...
    # Compute Precision, Recall, and F1-Score on the test split
    test_pred = out[split_idx['test']].argmax(dim=-1, keepdim=False).cpu().numpy()
    test_true = dataset.label[split_idx['test']].squeeze().cpu().numpy()
    ...
    precision = TP / (TP + FP + 1e-6)  # Per-class Precision
    recall = TP / (TP + FN + 1e-6)     # Per-class Recall
    f1 = 2 * (precision * recall) / (precision + recall + 1e-6)  # Per-class F1-score
